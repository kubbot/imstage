...
import { runDataset } from './benchmark/run.mjs';
...
export function createDatasetApi({dataDir, datasetDir=process.env.IMSTAGE_EVAL_DATASET_DIR ?? path.join(REPO_ROOT,'.local/datasets/chat-screenshot-edits-v1'), outDir=path.join(dataDir,'benchmark'), sendJson, readJsonBody}) {
  ...
  let execution={running:false,startedAt:null,finishedAt:null,error:null};
  ...
  return async function handle(req,res,url) {
    ...
    if(p.length===3 && p[2]==='run') {
      if(req.method==='GET'){sendJson(res,200,execution);return true;}
      if(req.method!=='POST')throw new AppError('not_found','接口不存在',404);
      await readJsonBody(req,2000);
      if(execution.running)throw new AppError('already_running','整套评测正在执行，请等待结果',409);
      execution={running:true,startedAt:new Date().toISOString(),finishedAt:null,error:null};
      sendJson(res,202,execution);
      runDataset({datasetDir,outDir,writeExpected:true}).then(report=>{execution={...execution,running:false,finishedAt:new Date().toISOString(),passed:report.ok,counts:report.counts};}).catch(()=>{execution={...execution,running:false,finishedAt:new Date().toISOString(),error:'评测未完成，请检查服务端配置与日志'};});
      return true;
    }
