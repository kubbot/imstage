...
export const BENCHMARK_PROMPT_VERSION = 'screenshot-edit-v2';
export const BENCHMARK_RUNTIME_VERSION = 'edit-render-score-v2';
...
export async function runDataset({
  ...
  const callGenerate = generatePlan ?? callDeepSeekPlan;
  const callRender = renderPlan ?? renderEditPlan;
  ...
  for (const caseData of selected) {
    ...
    try {
      ...
      const generated = await callGenerate({ config, request, signal: undefined });
      ...
      const rendered = await renderCase({ callRender, caseData, sourceBuffer, assets, plan });
