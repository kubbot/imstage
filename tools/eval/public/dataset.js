...
function render(){
 ...
 $('tool-trace').replaceChildren();for(const t of c.run?.toolTrace||[]){const p=document.createElement('p');p.textContent=t.type==='assistant'?t.text:`${t.name} · ${t.state} · ${t.detail}`;$('tool-trace').append(p);}
...
async function pollRun(){try{const status=await request('/api/dataset/run');$('run-agent').disabled=status.running;text('run-state',status.running?'Agent 正在逐例执行…':status.error||'');if(status.running){await load();setTimeout(pollRun,5000);}else await load();}catch(e){text('run-state',e.message);$('run-agent').disabled=false;}}
$('run-agent').onclick=async()=>{if($('run-agent').disabled)return;$('run-agent').disabled=true;try{await request('/api/dataset/run',{method:'POST',body:'{}'});await pollRun();}catch(e){text('run-state',e.message);$('run-agent').disabled=false;}};
pollRun();
