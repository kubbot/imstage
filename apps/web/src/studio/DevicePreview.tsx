...
export default function DevicePreview({ scene, full, children, zoom = 'fit', onScale }: {
  ...
  return <div className="device-preview" ref={host} style={{ minWidth: (width + 16) * scale }}><div className="agent-phone device-frame" data-mode={full ? 'full' : 'standard'} data-surface={scene.surface || 'ios'} style={{ width: width + 16, height: scene.reference || full ? 'auto' : profile.height + 16, zoom: scale }}>{children}</div></div>;
}
