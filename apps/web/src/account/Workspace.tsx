...
export default function Workspace() {
  ...
  return <div className="workspace-shell">
    <aside className="workspace-sidebar"><span className="account-kicker">{a.workspaceKicker}</span><a href="#/workspace" className="workspace-nav active" aria-current="page"><IconLayoutGrid size={19} /> {a.navWorks}</a><a href="#/projects" className="workspace-nav"><IconLayoutGrid size={19} /> {a.navProjects}</a><a href="#/templates" className="workspace-nav"><IconMessageCircle size={19} /> {a.navInspiration}</a><a href="#/studio" className="workspace-nav"><IconEdit size={19} /> {a.navLocalDraft}</a><a href="#/account" className="workspace-nav"><IconSettings size={19} /> {a.navAccount}</a><div className="workspace-sidebar-note"><IconDeviceFloppy size={20} /><p>{a.sidebarNote}</p></div></aside>
    <section className="workspace-content"><header className="workspace-heading"><div><span className="account-kicker">{a.workspaceStory}</span><h1>{a.workspaceTitle(user?.name || '')}</h1><p>{a.workspaceLede}</p></div><a className="btn btn-primary" href="#/workspace?scene=new"><IconPlus size={18} /> {a.newConversation}</a></header>
      <div className="workspace-start"><div><span className="account-kicker">{a.startKicker}</span><h2>{a.startTitleA}<br />{a.startTitleB}</h2><p>{a.startBody}</p><a href="#/workspace?scene=new" className="text-link">{a.openEditor} <IconArrowRight size={18} /></a></div><div className="workspace-mini" aria-hidden="true"><span className="workspace-mini-title">{a.miniTitle}</span><div>{a.miniLineA}</div><div>{a.miniLineB}</div><span>{a.miniInspiration}</span></div></div>
