...
export const SITE_COPY: Record<Locale, SiteCopy> = {
  zh: {
    ...
    footer: { tagline: '给每段对话，一个舞台。', templates: '模板', docs: '使用与接入', github: 'GitHub', license: 'MIT License', note: '为设计、教学与虚构叙事而作' },
  ...
  en: {
    ...
    footer: { tagline: 'A stage for every conversation.', templates: 'Templates', docs: 'Docs', github: 'GitHub', license: 'MIT License', note: 'Made for design, teaching and fiction' },
...
export const LANDING_COPY: Record<Locale, LandingCopy> = {
  zh: {
    ...
    capabilities: [
      ...
      { title: 'Agent 与 MCP', detail: '在网页或 ChatGPT 中创作，继续编辑同一份作品。' },
    ],
    ...
    faq: [
      ...
      { q: '托管服务和 MCP 怎么收费？', a: '项目采用 MIT 许可，浏览器本地编辑免费。托管站点已可注册使用；AI 能力需要服务端配置模型。ChatGPT 可通过账号授权接入，当前需要手动添加。付费套餐与计费尚未推出。' },
    ],
    ctaTitle: '下一段对话，等你开场。',
  ...
  en: {
    ...
    capabilities: [
      ...
      { title: 'Agent & MCP', detail: 'Create on the web or in ChatGPT, then keep editing the same scene.' },
    ],
    ...
    faq: [
      ...
      { q: 'How are hosting and MCP priced?', a: 'The project is MIT-licensed and local editing is free. The hosted site is open for registration; AI features need a model configured on the server. Connect ChatGPT with your account; manual setup is currently required. Paid plans and billing are not available yet.' },
    ],
