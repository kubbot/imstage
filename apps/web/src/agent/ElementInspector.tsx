...
import { applyFictionalMark, FICTIONAL_MARK_LABEL } from '../../../../packages/schema/fictional-mark.mjs';
...
export default function ElementInspector({ scene, selected, locked, onSelect, onChange, onBusy }: Props) {
  ...
  const en = locale === 'en';
  const markLabel = en ? 'Show the “fictional conversation” label' : '显示「虚构对话」标记';
  const markRefNote = en ? 'Screenshot-preserve mode: labels embedded in the original pixels cannot be removed by this switch.' : '截图保留模式：原图内嵌文字无法通过此开关移除。';
  const markCustomNote = en ? 'This scene has a custom watermark; turning the label off keeps it.' : '当前场景已有自定义水印，关闭标记不会删除它。';
  ...
  return <div className="agent-inspector property-inspector"><fieldset disabled={locked}>
    ...
    {person ? <>
      ...
      <Section title={e.frameSection}>{text(e.headerText, 'headerText', scene.headerText ?? (scene.participants.length <= 2 ? scene.participants.find(p => p.id !== scene.selfId)?.name || scene.title : scene.title))}<div className="inspector-pair">{text(e.deviceTime, 'deviceTime', scene.deviceTime)}<label>{e.battery}<input type="number" min={0} max={100} value={scene.battery ?? 80} onChange={e => update({ battery: Number(e.target.value) })}/></label></div><label>{e.storyToday}<input aria-label={e.storyToday} type="date" value={scene.referenceDate || ''} onChange={e => update({referenceDate:e.target.value || undefined})}/></label>{scene.messages.some(m=>m.date) ? <p className="property-help">{e.dateHelp}</p> : text(e.dateText, 'date', scene.date)}{text(e.composerText, 'composerText', scene.composerText ?? '')}<label className="inspector-check"><input type="checkbox" aria-label={markLabel} disabled={!!scene.reference} checked={scene.watermark === FICTIONAL_MARK_LABEL} onChange={ev => update({ watermark: applyFictionalMark(scene.watermark, ev.target.checked, FICTIONAL_MARK_LABEL) })}/> {markLabel}</label>{scene.reference && <p className="property-help">{markRefNote}</p>}{!scene.reference && scene.watermark && scene.watermark !== FICTIONAL_MARK_LABEL && <p className="property-help">{markCustomNote}</p>}{text(e.watermark, 'watermark', scene.watermark)}</Section>
