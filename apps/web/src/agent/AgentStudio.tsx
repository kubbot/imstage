...
export default function AgentStudio({ initialScene, initialProjectId, onProjectChange, onSceneChange, accountAction, persistLocal = true, disabled = false, onBusyChange, onStorageError }: Props) {
  ...
  async function send(event?: FormEvent, edit = false) {
    ...
    const snapshot = liveScene.current; const id = crypto.randomUUID(); const targetId = edit && selected ? selected : undefined;
  ...
  return <section className="agent-workspace" aria-label="Agent 对话创作台" data-tab={tab} data-edit={drawer}>
    <header className="agent-heading"><div><IconSparkles size={20} /><h1>对话创作</h1><span>把想法聊成画面</span></div><div className="agent-heading-actions">{user && accountAction?.(scene, locked, projectId)}<button className="agent-button" disabled={locked || !history.length} aria-label="撤销上次修改" onClick={() => { const old = history.at(-1); if (old) { setScene(old); setSelected(''); setHistory(h => h.slice(0,-1)); setNotice('已撤销画面修改。'); } }}><IconArrowBackUp size={17} /></button><details className="agent-more"><summary>更多</summary><button onClick={() => downloadText('imstage-scene.json', JSON.stringify(scene, null, 2))}>下载场景 JSON</button></details></div></header>
    ...
    {exportScene && <div className="agent-export" aria-hidden="true"><div ref={exportRef} className="agent-export-frame" data-mode={full ? 'full' : 'standard'} style={{ width:exportScene.surface === 'desktop' ? 900 : 360, height:full ? 'auto':640, overflow:'hidden' }}><SceneView scene={exportScene} exportMode/></div></div>}
  </section>;
}
