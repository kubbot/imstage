...
import { IconCopy, IconUpload, IconArrowUp, IconCheck, IconDownload, IconMessageCircle, IconPaperclip, IconPlayerStop, IconSparkles, IconWand, IconX, IconArrowBackUp, IconArrowForwardUp, IconLayoutSidebarLeftCollapse, IconListDetails, IconPlus, IconMinus, IconLoader2 } from '@tabler/icons-react';
...
export default function AgentStudio({ initialScene, initialProjectId, onProjectChange, onSceneChange, accountAction, persistLocal = true, disabled = false, onBusyChange, onStorageError }: Props) {
  ...
  const [tab, setTab] = useState<'chat'|'canvas'>(sample ? 'canvas' : 'chat');
  ...
  const [full, setFull] = useState(Boolean(sample));
