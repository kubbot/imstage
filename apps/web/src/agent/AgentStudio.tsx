...
export default function AgentStudio({ initialScene, onSceneChange, accountAction, persistLocal = true, disabled = false, onBusyChange, onStorageError }: Props) {
  ...
  return <section className="agent-workspace" aria-label="Agent 对话创作台" data-tab={tab} data-edit={drawer}>
    ...
    {exportScene && <div className="agent-export" aria-hidden="true"><div ref={exportRef} className="agent-export-frame" data-mode={full ? 'full' : 'standard'} style={{ width:360, height:full ? 'auto':640, overflow:'hidden' }}><SceneView scene={exportScene} exportMode/></div></div>}
  </section>;
}
