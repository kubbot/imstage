...
export default function AgentStudio({ initialScene, initialProjectId, onProjectChange, onSceneChange, accountAction, persistLocal = true, disabled = false, onBusyChange, onStorageError }: Props) {
  ...
  const [scene, setScene] = useState<Scene>(() => {
    ...
    return { ...createScene(), id: crypto.randomUUID(), title: '新的对话', surface:'ios',deviceProfileId:'iphone-15-pro', selfId: 'me', participants: [{id:'me',name:'我'},{id:'other',name:'对方'}], messages: [] };
  });
  ...
  async function send(event?: FormEvent, edit = false) {
    ...
    if(contacts.loading || contacts.error){setFailure('请先在人物与头像中完成读取或重试，确保生成时使用正确的人物配置。');setPeopleOpen(true);setDrawer(false);return;}
    const run = ++sequence.current; const abort = new AbortController(); controller.current = abort;
    const snapshot = edit ? liveScene.current : applySelfDefault(liveScene.current,contacts.library); const id = crypto.randomUUID(); const targetId = edit && selected ? selected : undefined;
    ...
    try {
      for await (const item of runAgent({ prompt: text, projectId:projectId||undefined, scene: snapshot, targetId, attachments: edit ? [] : attachments, history: previous }, user.id, abort.signal)) {
  ...
  return <section className="agent-workspace" aria-label="Agent 对话创作台" data-tab={tab} data-edit={drawer}>
    <header className="agent-heading"><div><IconSparkles size={20} /><h1>对话创作</h1><span>把想法聊成画面</span></div><div className="agent-heading-actions"><button className="agent-button" disabled={locked||!user} aria-expanded={peopleOpen} onClick={()=>{setPeopleOpen(!peopleOpen);setDrawer(false);}}>人物与头像</button>{user && accountAction?.(scene, locked, projectId)}<button className="agent-button" disabled={locked || !history.length} aria-label="撤销上次修改" onClick={() => { const old = history.at(-1); if (old) { setScene(old); setSelected(''); setHistory(h => h.slice(0,-1)); setNotice('已撤销画面修改。'); } }}><IconArrowBackUp size={17} /></button><details className="agent-more"><summary>更多</summary><button onClick={() => downloadText('imstage-scene.json', JSON.stringify(scene, null, 2))}>下载场景 JSON</button></details></div></header>
