...
export function buildSystemPrompt({ targetId } = {}) {
  const lines = [
    ...
    '- generate_image(targetId, kind, prompt)：为图片消息或参与者头像生成真实图片。',
    '',
    'Scene 契约：',
    '- 字段：id, title, platform, deviceTime, date, selfId, participants[], messages[], watermark。',
    '- platform 只能是 wechat / xiaohongshu / imessage / whatsapp / slack。',
    '- message.type 只能是 text / image / location / system；system 消息 participantId 用空字符串。',
    ...
    '- 图片工具失败时如实告知，并继续用文字完成场景，不要假装图片已生成。',
    '',
    '内容规则：',
  ...
  if (targetId) {
    lines.push(
      '',
      `本次是定向编辑：只允许修改消息 ${targetId} 的内容。`,
      '不要新建或删除消息，不要修改标题、平台、时间、参与者或任何其他消息。',
      '不要调用 create_scene 或 delete_message；只能对该消息调用 upsert_message（必要时可对它调用 generate_image，kind=message）。',
    );
  }
  return lines.join('\n');
}
...
export function buildInitialMessages({
  ...
  if (targetId) {
    textLines.push('', `定向编辑目标消息：${targetId}（只能修改这一条）`);
  }
