import {componentFrame,refineRectEdges} from './frame-components.mjs';
...
export async function findFrame(source,box) {
 ...
 const component=componentFrame(data,w,h,c,initial);if(component)component.pixels=refineRectEdges(data,w,h,c,component.pixels);if(component)return {...component,box:[component.pixels[0]/w*1000,component.pixels[1]/h*1000,component.pixels[2]/w*1000,component.pixels[3]/h*1000],coordinateSpace:'normalized_1000',sourceWidth:w,sourceHeight:h,note:'由源图连通区域测量，保持槽位不移动。'};
 ...
 return {method:'edge_estimate',confidence:null,coordinateSpace:'normalized_1000',sourceWidth:w,sourceHeight:h,box:found,pixels:[l.value,t.value,r.value-l.value,b.value-t.value],edgeStrength:[l,t,r,b].map(x=>Number(x.quality.toFixed(2))),note:'基于像素边缘测量，请对照原图确认；圆角不改变外接矩形。'};
}
